'use client';
import {
  PositionsDetailFormSchema,
  PositionsDetailNormalized,
  PositionsDetailNormalizedSchema,
...
interface Props {
  ...
  detailData: PositionsDetailNormalized;
}
...
export default function FieldsView({ mode, nameSubject, detailData }: Props) {
  ...
  async function onSave() {
    try {
      ...
      const formParsed = PositionsDetailFormSchema.safeParse(inputsData);
      ...
      const normalized = PositionsDetailNormalizedSchema.parse(formParsed.data);
  ...
  return (
    <Box className="modalViewDetail-block">
      <Grid className="modalViewDetail-head">
        <Typography variant="h4">Должность</Typography>
        {mode == 'view' ? (
