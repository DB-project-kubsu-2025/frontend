...
export async function PUT(
  ...
  try {
    ...
    const data: any = await apiFetch(`/supplies/suppliers/${idNum}/`, 'PUT', {
      body: JSON.stringify(body),
    });
    console.log('%%%', data);

    const res = NextResponse.json(
      data.status === 400 ? data.data : { message: 'Поставщик обновлён' },
      { status: data.status },
    );
    return res;
  } catch (err: any) {
...
export async function DELETE(
  ...
  const idNum = Number(id);
  try {
    const data: any = await apiFetch(`/supplies/suppliers/${idNum}/`, 'DELETE'); //{ status: 204, ok: true, data: null }
    console.log('%%%', data);
